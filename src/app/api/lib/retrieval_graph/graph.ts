    ...
    if (typeof searchConditions.drop !== 'string') {
      ...
      if (drop.min !== undefined || drop.max !== undefined || drop.sort) {
        ...
        whereConditions.push({
          AND: [{ heelStackHeightMm: { not: null } }, { forefootStackHeightMm: { not: null } }],
        })
        ...
        if (drop.min !== undefined) {
          ...
          const forefootMin =
            typeof searchConditions.forefootStackHeightMm !== 'string' && searchConditions.forefootStackHeightMm.min
              ? searchConditions.forefootStackHeightMm.min
              : 10
          const forefootMax =
            typeof searchConditions.forefootStackHeightMm !== 'string' && searchConditions.forefootStackHeightMm.max
              ? searchConditions.forefootStackHeightMm.max
              : 40
...
async function shouldLookupShoe(state: typeof StateAnnotation.State, config: RunnableConfig): Promise<string> {
...
async function shouldRetrieveDocs(state: typeof StateAnnotation.State, config: RunnableConfig): Promise<string> {
  // Check if the last message is from the user
  const messages = state.messages
...
const builder = new StateGraph(
  ...
  .addConditionalEdges(START, shouldLookupShoe, ['fetchShoeData', 'generateQuery'])
  .addConditionalEdges('fetchShoeData', shouldRetrieveDocs, ['generateQuery', 'respond'])

 ...
 * @returns The reordered documents
 */
export function applyRecencyBias(docs: Document[], recencyWeight: number): Document[] {
 ...
 */
function parseSitemap(sitemapContent: string): { url: string; lastmod?: string }[] {
...
export async function getVectorStore(
    ...
    const count = await prisma.bRREmbedding.count();
                ...
                entries.map(async (entry) => {
                    try {
                        ...
                        for (const doc of urlDocuments) {
                            ...
                            const existingDoc = await prisma.bRREmbedding.findFirst({
                                where: {
                                    metadata: {
                                        path: ['contentHash'],
                                        equals: doc.metadata.contentHash,
                                    },
                                },
                            });
                        ...
                        if (documentsToAdd.length > 0) {
                            ...
                            console.log(`Added ${documentsToAdd.length} documents from ${entry.url} to vector store`);
                        }
